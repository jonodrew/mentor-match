...
def setup_data(s3_resource, participants_as_json):
    ...
    s3_resource.Object(test_bucket, f"{data_uuid}/0.json").put(
        Body=(bytes(participants_as_json.encode("UTF-8")))
    )
    yield
...
def test_state_machine_quality(participants_as_json, setup_data, s3_resource):
    ...
    output = async_process_data_event_handler(
        {"data_uuid": data_uuid, "bonus": 0}, None
    )
    assert output == {"data_uuid": data_uuid, "bonus": 0}
    assert s3_resource.Object(test_bucket, f"{data_uuid}/{str(output['bonus'])}.json")
